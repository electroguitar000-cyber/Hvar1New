...
public class LanguageManager {
    ...
    public String getMessage(String key, int arenaId) {
        ...
        // Подставляем номер арены в префикс
        String prefix = cache.getOrDefault("prefix", "&#E64C4C&l[Htime{arena}] &r")
                .replace("{arena}", String.valueOf(arenaId));

        if (!key.startsWith("help.") && !msg.startsWith("&#E64C4C&l[Htime")) {
            msg = prefix + msg;
        }
    ...
    public String getMessage(String key, int arenaId, String... replacements) {
        String msg = getMessage(key, arenaId);
